  ...
  useEffect(() => {
    async function load() {
      ...
      if (responderIds.length > 0) {
        ...
        (profiles ?? []).forEach((p) => {
          profileMap[p.user_id] = { name: p.business_name || p.name || "Productor", rating: p.rating ?? null, verified: p.verified ?? false };
        });
      }

      setResponses(rows.map((r) => ({
        ...r,
        producer_name: profileMap[r.responder_id]?.name ?? "Productor",
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 p-3 sm:p-6 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <button
          type="button"
          onClick={() => router.push("/dashboard/exportador/rfq/nuevo")}
          ...
          {solicitudes.map((s) => {
            ...
            return (
              <div key={s.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                {/* Solicitud header — click to expand */}
                <button
                  type="button"
                      ...
                      <>
                          ...
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="bg-gray-50 border-b border-gray-100">
                                <th className="px-4 py-3 text-left text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">Productor</th>
                        ...
                        {solResponses.some((r) => r.notes) && (
                          <div className="px-5 py-4 border-t border-gray-50 space-y-2">
                            <p className="text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">Notas de productores</p>
